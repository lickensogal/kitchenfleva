...
export function createHomeRenderer(content) {
  function renderRecipes(filter = 'all') {
    const grid = $('#recipe-grid')
    if (!grid) return
    grid.innerHTML = content.recipes
      .filter((recipe) => filter === 'all' || recipe.category === filter)
      .map((recipe) => `
        <article class="recipe-card">
          <div class="recipe-photo" style="background-image:url('${escapeHtml(safeImageUrl(recipe.image || recipe.image_url))}')"></div>
  ...
  function renderStories() {
    ...
    grid.innerHTML = content.stories.map((story) => `
      <article class="story-card">
        <div class="story-image" style="background-image:url('${escapeHtml(safeImageUrl(story.image || story.image_url))}')"></div>
  ...
  function renderProducts() {
    ...
    if (!grid) return
    const covers = ['Everyday<br>Kenyan<br>Kitchen', 'Home<br>Baker’s<br>Notes', 'Cook More,<br>Stress Less']
    grid.innerHTML = content.products.map((product, index) => `
      <article class="product-card"><div class="product-cover">${product.cover || escapeHtml(product.cover_label || covers[index % covers.length])}</div>
